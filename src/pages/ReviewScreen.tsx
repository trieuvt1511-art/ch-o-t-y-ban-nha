      ...
      <div className="w-full max-w-[430px] px-5 pt-6 pb-24 page-enter">
        ...
        {reviewWords.length === 0 && !completed ? (
          <div className="text-center py-16">
            ...
            <button onClick={() => navigate('/scenarios')} className="mt-6 btn-primary">
              Học từ mới →
            </button>
          </div>
        ) : completed ? (
          ...
          <>
            <div className="mb-6">
              <div className="flex justify-between text-sm text-muted-foreground mb-2">
                <span className="font-bold">{currentIndex + 1} / {reviewWords.length}</span>
              </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {ratings.map(r => (
                    <button key={r.label} onClick={handleRate}
                      className={`${r.className} rounded-full min-h-[48px] text-xs font-bold hover:opacity-90 transition-opacity shadow-card`}>
                      {r.label}
                    </button>
                  ))}
                </div>
